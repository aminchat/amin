...
## 🖥 نسخه‌ی گرافیکی در مرورگر — NetMon GUI (پایتون)

همان ابزار، ولی با **محیط گرافیکی در کروم** + امکان **مدیریت**:
- نمایش زنده‌ی اتصال‌ها (اسم برنامه، آی‌پی، پورت، سرویس، وضعیت، کشور سرور)
- 🚫 **بلاک کردن اینترنت هر برنامه** با یک کلیک (قانون فایروال ویندوز)
- 🔓 آزادسازی بلاک با یک کلیک
- ✖ بستن (End Task) هر پروسه
- نمودار «بیشترین اتصال به تفکیک برنامه» + جستجو + بروزرسانی خودکار

> نکته: مرورگر به‌خاطر امنیت نمی‌تواند اتصالات سیستم را ببیند؛ برای همین یک برنامه‌ی پایتون محلی داده‌ها را می‌خواند و رابط گرافیکی را در مرورگر نشان می‌دهد. هیچ کتابخانه‌ی اضافه‌ای لازم نیست (فقط پایتون استاندارد).

### ۱) نصب پایتون (یک بار برای همیشه)
1. از **python.org/downloads** آخرین Python 3 را دانلود و نصب کن.
2. ⚠️ موقع نصب حتماً تیک **«Add python.exe to PATH»** را بزن.
3. تست: در CMD بنویس `python --version` — اگر نسخه چاپ شد درست است.

### ۲) اجرا (فقط یک خط)
```powershell
$f="$env:USERPROFILE\Desktop\netmon-gui.py"; Invoke-WebRequest -UseBasicParsing -Uri "https://raw.githubusercontent.com/aminchat/amin/arena/01a0292a-amin/network-monitor/netmon-gui.py" -OutFile $f; python $f
```
کروم خودکار باز می‌شود → `http://127.0.0.1:8124`
اگر raw.githubusercontent باز نشد، به‌جای آن لینک پیش‌نمایش (در گفتگو) را جایگزین کن.

### ۳) برای بلاک/بستن پروسه‌ها
این کارها دسترسی Administrator می‌خواهند — یکی از این دو راه:
- پاورشل را **Run as administrator** باز کن و همان دستور بالا را بزن؛ **یا**
- داخل خود صفحه، دکمه‌ی نارنجی «ری‌استارت با دسترسی Administrator» را بزن (پنجره‌ی UAC می‌آید).
...
## 📊 شمارش مصرف اینترنت هر برنامه (v1.1)

- دو کارت جدید بالای صفحه: **سرعت دانلود/آپلود کل سیستم + مجموع** (از کارت شبکه — بدون نیاز به ادمین)
- پنل جدید «مصرف اینترنت هر برنامه»: حجم **دانلود و آپلود جداگانه + سرعت لحظه‌ای هر برنامه** از لحظه‌ی شروع مانیتور + دکمه‌ی پایان دادن
- نحوه‌ی کار: از API داخلی ویندوز (GetPerTcpConnectionEstats) شمارنده‌ی بایتِ هر اتصال TCP خوانده می‌شود و به پروسه‌ی صاحبش نسبت داده می‌شود — بدون نصب هیچ درایوری
- **پیش‌نیاز:** اجرا با Administrator (دابل‌کلیک روی Start-NetMon.bat و تایید UAC همین کار را می‌کند)
- محدودیت‌ها: فقط TCP شمرده می‌شود (اگر مرورگر روی QUIC/UDP باشد شمرده نمی‌شود — در شبکه‌های ایران UDP معمولاً فیلتر است و ترافیک روی TCP می‌رود)؛ شمارش از لحظه‌ی شروع مانیتور است نه از اول روز
